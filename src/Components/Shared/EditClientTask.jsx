...
import { patchTask } from "@/lib/actions/admin";
import { getToken } from "@/lib/actions/tokenGet";
const EditClientTask = ({ tasks }) => {
  const defaultDeadline = tasks?.deadline
    ? parseDate(tasks.deadline)
    : undefined;
  ...
  const onSubmit = async (e) => {
    ...
    const res = await patchTask(tasks._id, data, token);
    if (tasks.status === "Open") {
      ...
      alert("You cannot edit a task that is not open.");
      return;
    }
  };
            ...
            <Modal.Body>
              <Form className="w-full max-w-96" onSubmit={onSubmit}>
                <Fieldset>
                  ...
                  <Description>Update your task information.</Description>
                  <FieldGroup>
                    <TextField name="TaskTitle" defaultValue={tasks.TaskTitle}>
                    ...
                    <Select
                      className="w-full"
                      name="category"
                      defaultSelectedKey={tasks?.category}
